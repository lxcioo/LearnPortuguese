...
const run = async () => {
    console.log("--- Audio Generierung (Neue Struktur) ---");
    const course = content.courses[0];

    // FIX: Iteration über UNITS statt LESSONS
    if (course.units) {
                    ...
                    if (level.exercises) {
                        for (const exercise of level.exercises) {
                            ...
                            // 3. Multiple Choice
                                ...
                                if (exercise.audioText) await downloadAudio(exercise.audioText, exercise.id);
